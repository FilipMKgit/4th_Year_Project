#this script evaluates xgboost classification performance by comparing predicted labels  
...
xgb_data_dir = base_dir / "RandomForest_data" / "XGBoost" #xgboost prediction outputs
...
################################################################
#function to evaluate xgboost predictions vs ground truth  

def evaluate_xgb(pred_files, gt_files):
    #computes accuracy, precision, recall, and confusion matrix for xgboost predictions
...
print("\nevaluating xgboost - predictions vs annotated data...") 
metrics_xgb, avg_conf_matrix, unique_labels = evaluate_xgb(load_csv(xgb_data_dir), load_csv(ground_truth_dir)) #compare xgboost results to ground truth

################################################################
#print performance results  

print("\n--- xgboost model performance ---") 
print(f"accuracy: {metrics_xgb['accuracy']:.4f}" if metrics_xgb["accuracy"] is not None else "accuracy: n/a") 
print(f"precision: {metrics_xgb['precision']:.4f}" if metrics_xgb["precision"] is not None else "precision: n/a") 
print(f"recall: {metrics_xgb['recall']:.4f}" if metrics_xgb["recall"] is not None else "recall: n/a") 
