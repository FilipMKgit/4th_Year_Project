#this script tests different min_dist values for UMAP on a random subset of standardized data
#it selects 5 random files and applies UMAP using multiple min_dist values
...
min_dist_values = [0.0, 0.1, 0.25, 0.5, 0.8, 0.99]  #different min_dist values to test
...
print("executing UMAP transformation...") #execution message
...
for file_path in selected_files: #loop through randomly selected files
    ...
    ################################################################
    # apply UMAP for each min_dist value
    
    for min_dist in min_dist_values: #loop through different min_dist values
        output_dir = output_base_dir / f"UMAP_test_minDist={min_dist}" #define output directory for current min_dist
        output_dir.mkdir(parents=True, exist_ok=True)  #make sure directory exists

        #initialize and apply UMAP
        reducer = umap.UMAP(n_neighbors=15, min_dist=min_dist, n_components=2, random_state=42) #set UMAP parameters
        ...
        output_file_name = file_path.stem.replace("_standardized", "") + f"_UMAP-transformed_minDist={min_dist}.csv" #modify filename
...
print('use the viz_test_UMAP script to determine minDist for final script') #print hint for next steps