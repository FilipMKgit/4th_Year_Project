#this script visualizes clustered data from annotated cytometry files (including noise)
...
output_dir = Path("C:/Users/filip/OneDrive/Desktop/4YR_PROJ/work_data/annotated_data_with_noise") #path to data folder
...
def select_file():
    ...
    files = list(output_dir.glob("*.csv")) #get csv files
...
def visualize(file, x, y):
    ...
    species = data[data["Cluster"] != -1]

    #assign colours for species
    unique_clusters = sorted(species["Cluster"].unique())
    cmap = plt.get_cmap("turbo")
    cluster_colours = {cluster: cmap(i / max(1, len(unique_clusters) - 1)) for i, cluster in enumerate(unique_clusters)}
    ...
    plt.title(f"Species Visualization (Including Noise): {sample_id}")
...
if __name__ == "__main__":
    print("cluster data visualization (including noise)")
    file = select_file()
    if file:
        print(f"selected file: {file.name}")
        data = pd.read_csv(file)
        x, y = select_params(data)
        visualize(file, x, y)